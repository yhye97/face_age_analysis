...
KEY = 'ee1aadd3b85640209f66c907cdb75cf4'  # Replace with a valid subscription key (keeping the quotes in place).
CF.Key.set(KEY)
...
f = open('asian_actors.csv', 'r', encoding='utf-8')
rdr = csv.reader(f)
    ...
    while getattr(t, "do_run", True):
        ...
        if(index<len(white_actors)):
            if(index==len(white_actors)-1):
                ...
                iterator = white_actors[index:index+10]
                print(iterator)
                myPeriodicFunction(iterator)
        else:
            write_csv()
...
def write_csv():
    with open("asian_actors_face_api.csv", 'w', encoding='utf-8') as resultFile:
